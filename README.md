...
- **محور الزخم كان أعمى، وصار يقيس الدقيقة والخمس دقائق:** المحور كان يحسب
  `50 + (تغيّر الساعة × 5) + (تغيّر 24 ساعة × 1)` — لكن المزوّد كان يطلب من GMGN
  مفاتيح لا وجود لها (`price_change_1h` و`price_change_percent_1h` بشريط سفلي
  زائد)، بينما الحمولة الحقيقية تحمل `price_change_percent1h` مسطّحةً في القوائم،
  أو كائناً `price` فيه **مستويات** النوافذ (`price.price_1m` بجانب `price.price`).
  لا شيء تطابق ⇒ كل نافذة صارت `0.0` ⇒ **`base = 50` ثابتة لكل عملة فُحصت منذ
  البدء**، والمحور كله كان مقياس ضغط شراء يرتدي قبعة الزخم (علمه الخاص كان يطبع
  `1h=+0.00% 24h=+0.00%` في سجلّك)، و`momentum_positive = pc1h >= 0` كانت
  **صحيحة لكل عملة** لأن `0 >= 0`.
  الآن: (أ) المزوّد يقرأ التهجئات الثلاث ويشتقّ النسبة من المستويات حين لا يوجد
  حقل نسبة؛ (ب) النوافذ المُحتسبة صارت **دقيقة وخمس دقائق** بقرار المالك —
  لأن التفريغ في عملة pump.fun يقع خلال دقائق، فالنافذة الطويلة لا ترى الانعطاف؛
  (ج) الساعة و24 ساعة تُجلب وتُعرض «سياقاً» في الأعلام لكنها **لا تدخل النقاط**؛
  (د) نافذة لا يمكن قياسها تسهم بصفر وتُعلن `n/a` — **لا تُحتسب صفراً**، لأن الصفر
  ادّعاء أن السعر لم يتحرك. والوزنان ملكك:
  `market_analysis.momentum.weight_1m` (8.0) و`weight_5m` (3.0).
  النتيجة العملية على عملة واحدة: صاعدة `1m=+5.93% 5m=+25.00%` ⇒ **88**،
  وهابطة `1m=-7.41% 5m=-20.00%` (رغم `1h=+68%` و`24h=+292%` وضغط شراء 69%) ⇒
  **28**. تحت الكود القديم كانت العملتان **سواءً**، لأن السعر لم يكن يُقرأ أصلاً —
  وهذا أحد الأسباب التي جعلت عملات الـrug تُقبل: لحظة الضخّ تبدو «زخماً».
  ومحور **هيكل السوق** كان مصاباً بالعطب نفسه (`float(... or 0)` على نافذة الساعة):
  صار الحدّ `1h` يُستبعد من المتوسط حين لا يمكن قياسه ويُعلن
  `1h_momentum=UNKNOWN` بدل `+0.00%` المخترعة، وأول عينة بلا نافذة تأخذ 35
  المحافظة بدل أن تتظاهر بأن الساعة كانت مسطّحة.
- **`doctor` في نسخة النقل كان يطبع عنوان البطاقة كبصمة للنسخة:** نسخة النقل تُشحَن
  بلا `.git` عمداً (كي لا يُحيي `git checkout` دفترَ حسابات قديماً)، فيقرأ البند
  `code_revision` ملف `TRANSFER_REVISION.txt` — وكان يأخذ **أول 12 حرفاً من الملف**،
  أي عنوانه: `✔ commit ENZO — حزمة `. صحّ أخضر لا يسمّي شيئاً، وهو بالضبط ما
  تحتاج أن تراه عند التشخيص عن بُعد. صار يبحث عن الـsha الحقيقي (سطر
  `الالتزام :` أولاً، ثم أيّ رمز ستعشري 7–40، ثم أول سطر غير فارغ)، وإن لم يجد
  شيئاً أعاد `cannot identify the running code` بصراحة.
...
- **تدفق النشاط يعرض نوافذ الزخم وراء النتيجة، لا النتيجة وحدها:**
  `⏱ 1m -7.41% · ⏱ 5m -20.00% · scored: 1m + 5m · context (not scored) 1h +68.00% · 24h +292.00% · buy pressure 69.0%`.
  السبب: رقم الزخم وحده لا يُحكَم عليه — **28 قد تعني «تهبط الآن» وقد تعني «لا
  نافذة قابلة للقياس»** — والفرق بينهما مال حقيقي. تُحفظ النوافذ في سجلّ التدقيق
  (`data/enzo-audit.jsonl`) فتُقرأ لاحقاً بلا إعادة سؤال GMGN، وما لا يمكن قياسه
  يبقى `n/a` — **لا يُطبع `0.00%` أبداً**.
- **أوزان المحاور الستة على الصفحة = `weighted_confidence` في إعدادك حرفياً**
  (30/20/20/15/10/5 في الملف المشحون). كانت أرقاماً صلبة في القالب توافق الملف
  المشحون بالصدفة؛ فلو غيّرتَ وزناً لقَرأتَ صفحةً تصف تنقيطاً لا يفعله البوت —
  والاختبار يقارن البطاقات الست بالإعداد فعلاً.
...
المفتاح بعودته (لا إنذار دائم على بوت سليم)، ونوافذ الزخم (1m/5m المحتسبة
و1h/24h السياق) واصلةً إلى التدفق مع قرار مرفوض حقيقي، ومقارنة أوزان البطاقات
الستة بـ`weighted_confidence` في الإعداد.
...
market_analysis:               # عتبات الاكتشاف — حدّدها المالك
  ...
  momentum:                    # نوافذ الزخم — قرار المالك 2026-09-05
    weight_1m: 8.0             # نقاط تُضاف إلى 50 المحايدة لكل +1% في الدقيقة
    weight_5m: 3.0             # ونقاط لكل +1% في الخمس دقائق
...
عشرون حزمة، **891 تحقّقاً**، كلها تعمل بلا إعداد وبلا شبكة وبلا محفظة حقيقية:

```bash
python3 tests/test_dashboard_e2e.py        # 139  اللوحة: خادم حي + نقر كل زر في DOM
