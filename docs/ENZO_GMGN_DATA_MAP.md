...
### الشكل الموثَّق (v1.6.1) — وكان مصدراً لعطلين

`skills/gmgn-market/SKILL.md` §"market kline Response Fields": الاستجابة **كائن**
فيه مصفوفة `list`، وكل صف **كائن** أرقامه **نصوص** ووقته **ميلي-ثانية**:

```json
{"code":0,"data":{"list":[{"time":1775184000000,"open":"0.0000081",
  "close":"0.0000088","high":"0.0000090","low":"0.0000075",
  "volume":"1214","amount":"5379110"}]}}
```

`volume` = **دولارات**، `amount` = **وحدات من العملة** (الاسمان مضلّلان — لا تخمّن).

كان `kline()` يعيد `data` كما هي، فنشأ عطبان:

1. **مع النسخة الحقيقية:** يعيد **قاموساً** `{"list": [...]}` ⇒ كل مستهلك يرى
   `len()==1` ويستسلم ⇒ **محور الشموع كان أعمى بصمت** (لا خطأ في السجل).
2. **مع صفوف مصفوفات** (بنى أقدم / النسخة المحاكية): يعيد قائمة قوائم ⇒ أول
   `c.get("close")` في `market_structure` يرمي `AttributeError` ⇒ الاستثناء يهرب
   من المحور إلى `analyze` إلى `run_pipeline` ⇒ **القرار كله يصير
   `ANALYSIS_ERROR` و`hard_reject: ['EXCEPTION']`** — ويحدث هذا **من النظرة
   الثانية على العملة فصاعداً** فقط (الأولى ترجع مبكراً بـ
   `insufficient_samples`)، أي بالضبط في العملات التي يعيد المحرك فحصها، فيبدو
   الرفض كأنه عطل في مصدر البيانات لا خطأ في الكود.

**العلاج:** `gmgn._norm_kline()` يفكّ الغلاف (`data.list` أو `data.kline/candles/items`)
ويطبّع الصفوف — قاموساً كانت أو مصفوفة — إلى قائمة قواميس بقيم `float` و`time`
بالثواني، ولا يعيد قاموساً أبداً. و`market_structure._kline_volume_trend()` صار
**لا يرمي استثناءً إطلاقاً** (محور من ستة لا يجوز أن يُسقط قراراً)، والنسخة
المحاكية تُصدر الشكل الموثَّق افتراضياً (`{"kline_arrays": true}` يعيد الشكل
القديم للاختبار).
