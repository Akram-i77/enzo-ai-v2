...
```bash
for t in tests/test_*.py; do python3 "$t"; done    # 20 حزمة · 892 تحقّقاً (~4 دقائق)
```
...
في نسخة النقل (بلا `.git`) يُظهر `test_executor` **44** بدل 48 (المجموع **892**
بدل **896**): التحقّقات الأربعة الباقية تقارن السلوك بالكود القديم المسترجَع من
